...
(defn viz
  ...
  (let [{:keys [selector]
        ...
        no-node-labels (some :ignore-node-labels [cli-opts plugin-config])
        no-node-shapes (some :ignore-node-shapes [cli-opts plugin-config])
        no-seed-keys   (some :ignore-seed-keys   [cli-opts plugin-config])
